...
A verifier is given a chain and a set of revoked link identifiers.
...
7. No link's identifier is in the revoked set.
...
- the **request**: any of an event kind, a channel, a peer, and a tool name;
...
4. For each of `channel`, `kind`, `peer`, and `tool` present in the leaf caveats, the request states that attribute and its value is a member of the permitted set.
...
Rule 4 fails closed.
A request that does not state an attribute the mandate constrains MUST be denied.
A verifier MUST NOT treat an unstated attribute as evidence that the action does not touch that dimension.
...
A link identifier is revoked by a **kind:50002** event naming it.
A revocation MUST be honoured only when its author is the `issuer` of the named link or the chain's root authority.
...
A relay that does enforce mandates MUST use its own clock, MUST apply the actor check in Authorization rule 1 against `event.pubkey`, and SHOULD maintain the revoked set from kind:50002 events within its own community boundary.
...
Machine-readable vectors covering canonical encoding, rejection, narrowing, authorization, and signed chains are at `crates/buzz-mandate/tests/vectors.json`.
